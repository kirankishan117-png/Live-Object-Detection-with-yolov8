...
We can able to detect the live object by facing the camera
Live Object Detection with YOLOv8
Real-time object detection using Ultralytics YOLOv8, running on either your webcam or a video file, with results shown live in an OpenCV window (bounding boxes, labels, confidence, FPS).
...
5. Notes
Pretrained model (yolov8n.pt etc.) detects the 80 COCO classes (person, car, dog, chair, laptop, etc.) out of the box — no training needed.
